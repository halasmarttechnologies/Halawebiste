...
// Inline SVG social brand icons — eliminates the entire react-icons/fa6 sub-bundle
const SocialIcons = [
  {
    color: '#E1306C',
    svg: (
...
function IconItem({ index, initialOffset, amplitude }: { index: number; initialOffset: number; amplitude: number }) {
  const time = useTime();
  const iconDef = SocialIcons[index % SocialIcons.length];
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        rotate: rotation,
        backgroundColor: iconDef.bg ?? '#fff',
        color: iconDef.bg ? 'white' : iconDef.color,
      }}
    >
      {iconDef.svg}
    </motion.div>
